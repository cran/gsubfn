...
fn$by(CO2df[4:5], CO2df[2], x ~ coef(lm(uptake ~ ., x)), simplify = rbind)
...
library(grid)
CO2df <- as.data.frame(CO2)
print(fn$xyplot(uptake ~ conc | Plant, CO2df,
...
<<gsubfn-fn-simplify>>=
CO2df <- as.data.frame(CO2)
fn$by(CO2df, CO2df$Treatment, d ~ coef(lm(uptake ~ conc, d)), simplify = rbind)
@
...
The calculation is repeated using \code{dcast} from the \pkg{reshape2}
...
CO2df <- as.data.frame(CO2)
fn$aggregate(CO2df[4:5], CO2df[3], ~ mean(range(x)))

library(reshape2)
fn$dcast(Type ~ variable, data = melt(CO2df, id = 1:3), ~~ mean(range(x)))

library(doBy) # need version 0.9 or later
fn$summaryBy(. ~ Treatment, CO2df[3:5], FUN = ~~ c(midrange = mean(range(x))))
@
...
library(reshape2)
fn$dcast(Treatment ~ variable, data = melt(CO2df, id = 1:3), ~~ mean(range(x)))
...
and
\pkg{reshape2} \Sexpr{gsub("-", "--", packageDescription("reshape")$Version)}.
